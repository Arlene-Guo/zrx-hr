    ...
    util.addEvent($container, 'mousedown', function (event, target) {
        ...
        //NEXT MONTH BUTTON
            if (!self.disableNext && self.publish('view-changed', self, ['next-month']) !== false) {
                var monthNum = 1;
                if (self.settings.directShow) {
                    monthNum = self.settings.months;
                }
                self.viewStartDate.add('months',monthNum);
                self.draw();
            }
            return false;
        ...
        //PREVIOUS MONTH BUTTON
            if (!self.disablePreviousMonth && self.publish('view-changed', self, ['previous-month']) !== false) {
                ...
                self.viewStartDate.subtract('months',monthNum);
                self.draw();
            }
            return false;
...
Kalendae.prototype = {
	...
	selectMulDays : function (startDate, endDate, rule,dateList) {
		...
		do{
			if(!(rule(day))) {
				if(dateList && dateList[day.format("YYYY-MM")] && dateList[day.format("YYYY-MM")].join(",").indexOf(day.format("YYYY-MM-DD")) != -1);
				else days.push(day.format('YYYY-MM-DD'));
			}
			day.add('days', 1);
		}while(day <= endDay);
		if(days.length > 0){
...
Kalendae.Input = function (targetElement, options) {
    ...
    util.addEvent($input, 'blur', function () {
        ...
        else if(!self.settings.directShow){
            self.hide();
        }
    });
	...
	show : function () {
		...
		switch (opts.side) {
			...
			case 'bottom':
				/* falls through */
			default:
				style.left = (pos.left + (this.settings.noOffsetLeft || this.settings.offsetLeft)) + 'px';
