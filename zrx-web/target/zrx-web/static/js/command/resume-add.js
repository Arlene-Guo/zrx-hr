...
//			});
				...
				if(files && fn) {
					...
					$.ajax({
						...
						success: function(json) {
							...
							$('.resume-box').html('<a href="http://10.13.112.24:8080/zrx-web/file/downLoadFile?filepath=' + json.data + '&filename='+ files[0].name+'">' + files[0].name + '</a>');
						},
						error: function() {
							showTip('图片上传失败，请稍后重试');
						}
		...
		getAction: function() {
			if(this.devcfg) {
				return {
					showData: Common.action().Common.action.portalUrl + "resume/searchResume",
