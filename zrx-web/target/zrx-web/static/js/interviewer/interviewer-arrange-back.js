...
(function($, window) {
	...
	$.extend(List.prototype, {
					...
					success: function(json) {
						if(json.success) {
							...
							if(data.state == 3 || data.state == 4 || data.state == 5 || data.state == 6) {
								...
								html += '</div>';
							} else if(data.state == 7 || data.state == 8 || data.state == 9 || data.state == 10 || data.state == 11) {
								html += '<div class="row cl row4">';
								html += '<label class="form-label col-xs-3 col-sm-2">面试安排:</label>';
								html += '<span>初试</span>';
								html += '<label class="ml-30">面试类型:</label>';
		...
		saveData: function() {
			...
			if(self.state == 4 || self.state == 5 || self.state == 6) {
				data.state = $('input[name="sex"]:checked').val();
				remarksData.remarks= $('.initRemarks').val();
			} else if(self.state == 8 || self.state == 9 || self.state == 10) {
				data.state = $('input[name="sex"]:checked').val();
