...
(function($, window) {
	...
	$.extend(List.prototype, {
				...
				listener: {
					success: function(json) {
						if(json.success) {
							...
							tn.form.set($('#form-superAdmin-role-add'),data);
		...
		saveData: function() {
			...
			//data.phone = $('input[name=telephone]').val();
			//data.company = $('input[name=company]').val();
			//data.core = $('input[name=descript]').val();
			console.log(data);
			Ajax.request({
...
//					state: 1,
				...
				resultField: 'rows',
				attrs: ["userName", "jobNumber","id","phone","company","core"],
				maxShowCount: 0,
				autoComplete: true,
