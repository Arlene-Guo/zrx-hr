! function(window, document, undefined) {
	 ...
	 **/
	tn.url= {
		/**正则替换filebroke图片地址为外网地址，不切图: m.tuniucdn.com
		 *	如果后期增加参数，建议改成{}配置
		**/
		getPublicImage: function(url, type){//url:要处理的url，  type:正则配置
			if(typeof type != 'string'){
				type = 'filebroke';
			}
			var reg = {
					filebroke: /http:\/\/public-api.(bj|nj).pla.tuniu.org/g,
				}
			return url.replace(reg[type], 'http://m.tuniucdn.com');
		},
	},
	/**
	 * 类型判断
	 ...
	 */
	window.Cookie = tn.cookie = new function() {
	...
	/**
	 * Ajax操作
	 * serverTransfer 标识是否需要进行服务器中转，默认为false，false表示不进行中转直接请求接口地址，true表示需要进行服务器中转
	 */
	window.Ajax = tn.ajax = new function() {
		this.request = function(options) {
			var uid = tn.cookie.get("honeydukesUid");
			var nickname = tn.Base64.decode(tn.cookie.get("honeydukesUname"));
			var token = tn.cookie.get("honeydukesSessionID");
			return new function() {
				var _this = this;
				$.extend(true, this, {
					url: "",
				...
				for (var i in this.listener) {				
					var functionStr = 'if ("' + i + '" == "success"){' + 'var data = arguments[0], textStatus = arguments[1];' + 'if (this.encode){' + 'try{' + 'data = tn.Base64.decode(data);if(tn.json.encode(data).errorCode==10050){delete sessionStorage.role;}	' + '}catch (e){' + 'throw new Error("后台数据返回格式错误，请进行Base64编码处理！请求地址："+this.url+"返回数据："+data);' + '}' + 'try{' + 'data = tn.json.encode(data);' + '}catch (e){' + 'throw new Error("后台数据返回格式错误，请进行JSON格式检查！请求地址："+this.url+"返回数据："+data);' + '}' + '}else{' + 'data = tn.json.encode(data);' + '}' + 'if(!data.success){if(this.failback){' + 'this.listener["' + i + '"].call(this, data, textStatus);' + '}else{' + 'if(data.errorCode==10050){delete sessionStorage.data;alert("登录过期，请重新登录；")};return; ' + '}' + '}else{' + 'this.listener["' + i + '"].call(this, data, textStatus);' + '}' + '}else if("' + i + '" == "requestcomplete"){' + 'var data = arguments[0];' + 'try{' + 'data = tn.Base64.decode(data.responseText);' + '}catch (e){' + 'throw new Error("后台数据返回格式错误，请进行Base64编码处理！请求地址："+this.url+"返回数据："+data);' + '}' + 'try{' + 'data = tn.json.decode(data);' + '}catch (e){' + 'throw new Error("后台数据返回格式错误，请进行JSON格式检查！请求地址："+this.url+"返回数据："+data);' + '}' + 'this.listener["' + i + '"].call(this, data, textStatus);' + '}else if("' + i + '" == "beforesuccess"){' + 'var data = arguments[0], type = arguments[1];' + 'this.listener["' + i + '"].call(this, data, type);return data' + '}else if("' + i + '" == "error"){' + 'this.listener["' + i + '"].call(this, arguments);' + '}else{' + 'this.listener["' + i + '"].call(this, arguments);' + '}';
...
//							r: Math.random()
						...
						if (!this.serverTransfer) {
							...
							userData = tn.Base64.encode(serverHref + connector + tn.Base64.encode(tn.json.decode(userData)));
					...
					if (this.type === "POST") {
						 ...
						 */
						if (this.encode) {
							this.data = tn.Base64.encode(tn.json.decode(this.data));
						}
						if (this.serverTransfer) {
							if (serverHref.indexOf("?") !== -1) {
								connector = "&";
							};
							serverHref = tn.Base64.encode(serverHref);
...
$p.setHref = function(s, sRel) {
	...
	this._fragment = "";
	this._params = {};
	var ok = tn.uriTest.scheme.test(s);
	if (!ok) {
	...
	if (sRel) {
		this._hrefCache = null;
		s = String(s);
		var isAbsolute = tn.uriTest.absUri.test(s);
		if (isAbsolute) {
			this.setHref(s);
			return;
		}
	...
	function parsePathAndRest(s) {
		...
		if (self._path.length > 0) {
			ok = tn.uriTest.dirPath.test(self._path);
			if (!ok) {
				throw new Error(0x0011, "URI.setHref(s,sRel):'" + s + "' is Not a well formatted URI.");
			}
			self._dirPath = RegExp.$1;
		}
		ok = tn.uriTest.fragment.test(s);
		if (ok) {
...
$p.toJSON = function() {
	...
	var json = tn.json.encode(tn.Base64.decode(query.replace('?', '')));
	for (var i in json) {
		var value = json[i];
