...
(function(window, document, undefined) {
	...
	var searchHtml = {
		...
		center: [
			...
			'<li class="searchRes" style="cursor: pointer;" extrakey="@<%=key%>#">@<%=key%>#</li>',
	...
	var defaults = {
		...
		suggestSelect: false, //li选择和回车键生效
	...
	var TNSearch = function TNSearch(o) {
		...
		this.opts = tn.util.merge(o || {}, TNSearch.defaults, defaults, {
			SID: tn.guid()
		});
	...
	tn.util.merge(TNSearch.prototype, {
		/**
		 * 初始化
		...
		renderSearchList: function(data) {
			...
			if (data) {
				...
				if(grid.opts.url && grid.opts.url.indexOf("venus") != -1 && grid.opts.url.indexOf("api/poi/district/search/") != -1 && data.result){
