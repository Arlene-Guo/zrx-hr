				...
				colModel: [
					/*{
						...
						handler: function(v,data,n){
							n.html('<input type="checkbox" style="margin:0 5px;" class="checkbox" name="resume" data-resumeId=' + data.resumeId + ' />');
							n.on('click','.checkbox',function(){
					...
					},
					{
						...
						width: 160,
						handler: function(v, data, n) {
//							n.append('<a href="javascript:;" class="edit">修改</a>');
...
//										success: function(json) {
//											if(json.success) {
//												var dataJson = json.data.rows;
//												var html = '<select style="width:100px;">';
//												for(var i = 0; i < dataJson.length; i++) {
//													html += '<option value="' + dataJson[i].dutiesNumber + '">' + dataJson[i].dutiesName + '</option>';
//												}
//												html += '</select>';
//												n.html(html + '<a href="javascript:;" style="margin-left:10px;" class="sure">确定</a>');
//											}
//										}
//									}
//								});
...
//								var data2={
//									id:data.resumeId,
//									intervieweeDutiesId:n.find('select option:selected').val(),
//									intervieweeDutiesName:n.find('select option:selected').html()
//								}
//								Ajax.request({
//									url: self.getAction().saveOrUpdateResume,
//									type: "GET",
//									data: data2,
//									listener: {
//										success: function(json) {
...
//							});
									...
									success: function(json) {
										if(json.success) {
											...
											if(data.resumeCommissionerId == '' || data.resumeCommissionerId == null) {
												...
												n.html('<div><span style="margin-right:10px;">' + data.resumeCommissionerName + '</span></div>');
											} else {
				...
				/*buttons:[{
					...
					operator: "assign",
					onpress: function(e) {
						self.assignCommissioner.call(self, e);
					}
				}], */
				css: {
					height: "auto",
		...
		assignCommissioner: function(e) {
			if($('input:checkbox[name=resume]:checked').length === 0){
				this.noty.error("请选择要批量处理的简历");
			} else {
				var reqParam = {};
				reqParam.assign=[];
				$('input:checkbox[name=resume]:checked').each(function(i){
					var resumeId = $(this).attr('data-resumeId');
					reqParam.assign.push(resumeId);
				 });
				var param = Base64.encode(JSON.encode(reqParam));
				layer_show('分配招聘专员', 'superAdmin-assign.html#'+param, '', '220');
			}
		}
	});
