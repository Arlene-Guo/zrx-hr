...
TNEditor.prototype.loadBody = function (){
	//先加载样式表
	$('head').append('<link type="text/css" rel="stylesheet" href="../../../1common/editor_css/base.css" />');
...
function editMenu(opt){
	var defaults = {
		imagePath : "../../../1common/editor_css/images/",
...
editMenu.prototype.menuInfo = {
	...
	action:{
		...
		addLink: function(e){
			...
			$(e.target).parent().append(self.popWin);
			self.popWin.show();
            //$("#" + self.editorKey, popBox).find('#closePopBoxBtn').click(function(){
			//	self.hidePopWin();
			//});
			$('#closePopBoxBtn',popBox).click(function(){	
				self.hidePopWin();
			});
...
editBody.prototype.setbaseSection = function() {
	...
	$('head',this.cw.document).append('<link rel="stylesheet" href="../../../1common/editor_css/editableIframe.css" />').append('<style>._0000FF {color:blue;}._FF0000 {color:red;} ._000000 {color:black;}</style>');
