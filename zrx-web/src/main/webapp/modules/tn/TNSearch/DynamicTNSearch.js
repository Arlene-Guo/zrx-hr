...
(function(window, document, undefined) {
	...
	var searchHtml = {
		//整体布局
		wrap: "<div class='searchDiv' style='display:none'><div class='searchHeader'></div><ul class='searchList'></ul><div class='searchFooter'></div></div>",
		//头部布局
		header: [''],
		//中间布局
		//带有title的搜索列表
	...
	var defaults = {
        ...
        handleDataCallBack: function(json){
            return json.data;//默认返回data数组
        }
	};
	 ...
	 * @return <void>
	 */
	var DynamicTNSearch = function DynamicTNSearch(o) {
		if (!this.dynamicTNSearch) return new DynamicTNSearch(o);
		if (o.param) {
			tn.util.merge(o.param, defaults.param);
		}
		this.searchHtml = tn.util.merge({}, searchHtml);
		this.opts = tn.util.merge(o || {}, DynamicTNSearch.defaults, defaults, {
			SID: tn.guid()
		});
		...
		//已选建议数据
		this.selectedData = [];
		this.dynamicTNSearch();
	};
	//控件自带参数表
	DynamicTNSearch.defaults = {};

	tn.util.merge(DynamicTNSearch.prototype, {
		/**
		 * 初始化
		 * @return {[type]} [description]
		 */
		dynamicTNSearch: function() {
			//初始化函数
			var opts = this.opts;
			//设置
			...
			this.nodes.wrapNode.attr('id', 'DynamicTNSearch_' + opts.SID)
		...
		dataTransform: function(data) {
			...
			if (grid.opts.handleData && tn.type.isFunction(grid.opts.handleData)) {
		...
		render: function() {
			...
			this.nodes.resultNode.append(tn.tmpl(this.searchHtml.center[2], {
		...
		renderSearchList: function(data) {
			...
			if (data) {
				...
				//无相关数据
				if (data.length == 0 || (data.result && data.result.length == 0)) {
					$('li#tempInputContent', grid.nodes.resultNode).html("无相关数据").show();
					grid.resetSearchEnv();
					return;
				}
				if(grid.opts.url && grid.opts.url.indexOf("venus") != -1 && grid.opts.url.indexOf("api/poi/district/search/") != -1){
					for (var i in data.result) {
						if (data.result[i] && tn.type.isObject(data.result[i])) {
							//结果集绑定数据
							html = $(tn.tmpl(grid.opts.showNode, data.result[i])).data({
								"data": data.result[i]
							});
					...
					for (var i in data) {
						if (data[i] && tn.type.isObject(data[i])) {
							//结果集绑定数据
							html = $(tn.tmpl(grid.opts.showNode, data[i])).data({
								"data": data[i]
							});
							grid.nodes.resultNode.append(html);
							//默认选中第一个
		...
		bindEvent: function(e) {
			...
			// 搜索触发事件
			targetNode[0].oninput = targetNode[0].onpropertychange = function() {
				grid.keyUpHandler();
			};
		...
		blurHandler: function(e) {
			...
			//用户自定义选择事件
			if (grid.opts.selectFn && tn.type.isFunction(grid.opts.selectFn)) {
		...
		showHandler: function(e, searchStr) {
			...
			this.nodes.wrapNode.css({
				...
				"margin-top": (searchElemetTop > 0 ? searchElemetTop*(-1) : 0)+"px"
			});
				...
				TNAjax.request({
					...
					listener: {
						success: function(json) {
							// $('li#tempInputContent', grid.nodes.resultNode).html("正在加载数据……").hide();
                            //调用用户自定义回调函数
                            var data = grid.opts.handleDataCallBack(json);
	...
	window.DynamicTNSearch = DynamicTNSearch;
