				...
				colModel: [{
								...
								listener: {
									success: function(json) {
										if(json.success) {
											...
											html += '<select name="" id="interviewerSelect" style="width:65%;margin-right:5px;">';
											...
											if(data.initInterviewerId == '' || data.initInterviewerId == null) {
												n.html('<div>' + html + '</div>');
												n.on('change', '#interviewerSelect', function() {
													var paramSave = {
												...
												n.html('<div><span style="margin-right:10px;">' + data.initInterviewerName + '</span><a class="edit">修改</a></div>');
												n.on('click', '.edit', function() {
													...
													n.on('change', '#interviewerSelect', function() {
														...
														var paramUpdate = {
															...
															initInterviewerName: n.find('select option:selected').text(),
															initInterviewerId: n.find('select').val()
														}
					...
					},
					{
						...
						handler: function(v, data, n) {
							...
							n.html('<div><span style="margin-right:10px;">' + result + '</span>'+'</div>');
						/*	n.html('<div><span style="margin-right:10px;">' + result
							+ '</span><a id="showResult" style="margin-right:5px">查看</a>' +
						'<a id="downloadRes">下载</a>'+'</div>');
							n.on('click','#showResult',function() {
								alert('简历查看');
							});
							n.on('click','#downloadRes',function() {
								alert('简历下载');
							}); */
						}
					},
