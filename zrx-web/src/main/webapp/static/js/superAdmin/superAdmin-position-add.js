...
(function($, window) {
	...
	$.extend(List.prototype, {
						...
						if(json.success) {
							...
							for(var i = 0; i < data.length; i++) {
								if(data[i].dutiesType=='职能'){
									data[i].dutiesType='职能';
								}else{
									data[i].dutiesType='非职能';
								}
				...
				listener: {
					success: function(json) {
						if(json.success) {
							...
							$('#dutiesDescription').val(data.dutiesDescription);
							/*
		...
		saveData: function(self) {
			...
			data.dutiesDescription = $('#dutiesDescription').val();
			...
			var suggest = new TNSearch({
				...
				selectFn: function(data) {
					...
					if(data!=''&&data!=null&&data.length!=0){
						...
						$('#dutiesDescription').val(data[0].dutiesDescription);
					}
				}
			});
